...
window.addEventListener('load', e => {
    ...
    createApp({
        ...
        sceneURL: params.load || 'sklady_test.gltf',
...
function runCode(app, puzzles) {
    ...
    /* ── Группы мешей по этапам сборки ── */
    var groups = {
        foundation: [
            'Бетон - 02',
            'Бетон - 04',
            'Плитка - Светло-Коричневая 15x15',
        ],
        frame: [
            'Металл - Сталь Нержавеющая',
        ],
        walls: [
            'Краска - Темно-Серая',
            'Краска - Глянцевая Белая',
            'Краска - Песок Сиенна',
            'Краска - Светло-Серая',
            'Краска_БАЛТ резина',
        ],
        office: [
            'Краска - Белила Титановые',
        ],
        roof: [
            'Краска - Антрацит',
        ],
        glazing: [
            'Стекло - Прозрачное Быстрое',
        ],
        territory: [
            'Дорожное Покрытие - Асфальт Светлый',
            'Дорожное Покрытие - Бетон 08 25',
            'Дерево - Сосна Продольная Горизонтальная',
        ],
    };

    /* ── Скрываем всё (кроме сферы окружения) ── */
    var allGroupMeshes = [];
    Object.keys(groups).forEach(function(key) {
        groups[key].forEach(function(n) { allGroupMeshes.push(n); });
    });

    allGroupMeshes.forEach(function(name) {
        var obj = app.scene.getObjectByName(name);
        if (obj) obj.visible = false;
    });

    /* ── Фундамент виден по умолчанию ── */
    groups.foundation.forEach(function(name) {
        var obj = app.scene.getObjectByName(name);
        if (obj) obj.visible = true;
    });

    /* ── Фон: скрываем Sphere001, WebGL прозрачный — цвет через CSS родителя ── */
    var bgSphere = app.scene.getObjectByName('Sphere001');
    if (bgSphere) bgSphere.visible = false;

    if (app.scene.background && app.scene.background.isTexture) {
        if (!app.scene.environment) app.scene.environment = app.scene.background;
        app.scene.background = null;
    }

    if (app.renderer) app.renderer.setClearColor(0x0F2B45, 0);

    /* ── Общий материал для стен (MeshStandard — гарантированный .color) ── */
    var paintMaterial = new v3d.MeshStandardMaterial({
      color: new v3d.Color(1.000, 0.757, 0.086), // Желтая по умолчанию
        roughness: 0.55,
        metalness: 0.05,
    ...
    groups.walls.forEach(function(name) {
        var obj = app.scene.getObjectByName(name);
        if (obj) obj.material = paintMaterial;
    });

    /* ── Обработчики сообщений от родительской страницы ── */
    window.addEventListener('message', function(e) {
        var data = e.data;
        if (!data || !data.type) return;

        /* Показать / скрыть группу */
        if (data.type === 'v3dSetGroupVisible') {
            var meshNames = groups[data.group];
            if (!meshNames) return;
            meshNames.forEach(function(name) {
                var obj = app.scene.getObjectByName(name);
                if (obj) obj.visible = data.visible;
            });
        }

        /* Переключить отдельный меш (временно для идентификации) */
        if (data.type === 'v3dToggleMesh') {
            var obj = app.scene.getObjectByName(data.name);
            if (obj) obj.visible = data.visible;
        }

        /* Сменить цвет стен */
        if (data.type === 'v3dSetColor') {
            paintMaterial.color.setRGB(data.r, data.g, data.b);
        }
